...
export default function VerificationDialog({
  ...
  const [verificationType, setVerificationType] = React.useState<'individu' | 'organisasi' | null>(
    null,
  );
  ...
  // WA OTP temporarily disabled — this pre-filled the WA step's phone field.
              ...
              <Stepper activeStep={activeStep} orientation="vertical" sx={{ p: 3 }}>
                  ...
                  <StepContent>
                    ...
                    <Box sx={{ mb: 2 }}>
                      <Button
                        ...
                        onClick={async () => {
                          if (!phone) {
                            showSnackbar('Nomor WhatsApp wajib diisi', 'warning');
                            return;
                          }
                          try {
                            setWaLoading(true);
                            ...
                            if (!markRes.success) {
                              showSnackbar(
                                markRes.error || 'Gagal menyimpan nomor WhatsApp',
                                'error',
                              );
                              return;
                            }
                ...
                <Step>
                  ...
                  <StepContent>
                    <Alert severity="info" sx={{ mb: 2, borderRadius: 2 }}>
                      Kode verifikasi (OTP) akan dikirim ke <b>{userEmail || 'email anda'}</b>. Cek
                      kotak masuk (Inbox), jika tidak ada periksa folder Spam/Promosi.
                    </Alert>
                        ...
                        onClick={async () => {
                          ...
                          try {
                            ...
                            if (res.success) {
                              ...
                              showSnackbar(res.error || 'Terjadi kesalahan', 'error');
                            }
                          } finally {
                      ...
                      <Button
                        ...
                        onClick={async () => {
                          if (emailOtp.length < 6) {
                            showSnackbar('Masukkan 6 digit kode OTP', 'warning');
                            return;
                          }
                          setEmailLoading(true);
                          try {
                            ...
                            if (res.success) {
                              ...
                              showSnackbar(res.error || 'Verifikasi gagal', 'error');
                            }
                          } finally {
                            setEmailLoading(false);
                          }
                ...
                <Step>
                  ...
                  <StepContent>
                        ...
                        <>
                          ...
                          >
                            {uploading
                              ? 'Sedang mengupload...'
                              : `Klik untuk upload ${
                                  verificationType === 'individu' ? 'foto KTP' : 'SK Kemenkumham'
                                }`}
                          </Typography>
                    ...
                    <StyledTextField
                      label={
                        verificationType === 'individu' ? 'Nomor NIK KTP' : 'Nomor SK Kemenkumham'
                      }
                      fullWidth
                      size="small"
                      placeholder={
                        verificationType === 'individu' ? '16 digit NIK' : 'Masukkan nomor SK'
                      }
                      sx={{ mt: 2 }}
                    ...
                    <Box sx={{ mb: 2, mt: 2 }}>
                      <Button
                        ...
                        onClick={async () => {
                          if (!docNumber) {
                            showSnackbar('Nomor dokumen wajib diisi', 'warning');
                            return;
                          }

                          if (verificationType === 'individu') {
                            if (docNumber.length !== 16) {
                              showSnackbar('Nomor NIK harus terdiri dari 16 digit angka', 'error');
                              return;
                            }
                          }
                          ...
                          if (res.success) {
                            ...
                            showSnackbar(res.error || 'Gagal menyimpan data verifikasi', 'error');
                          }
                        }}
                        sx={{
                          bgcolor: '#0ba976',
                      ...
                      <Button
                        ...
                        onClick={async () => {
                          ...
                          if (res.success) {
                            ...
                            showSnackbar(res.error || 'Gagal menyimpan alamat', 'error');
                          }
                        }}
      ...
      >
        <Box sx={{ p: 3, textAlign: 'center' }}>
          ...
          <Typography variant="body2" color="text.secondary" sx={{ mb: 1 }}>
            Kami telah mengirim kode OTP ke <b>{userEmail || 'email Anda'}</b>.
          </Typography>
          <Alert severity="warning" sx={{ borderRadius: 2, textAlign: 'left', mb: 3 }}>
            Cek kotak masuk (Inbox). Jika tidak ada, periksa folder <b>Spam/Promosi</b>.
          </Alert>
          <Button
            variant="contained"
            fullWidth
