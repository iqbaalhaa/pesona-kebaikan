...
export default async function AdminPage() {
  ...
  return (
    <Box sx={{ py: 2 }}>
      ...
      <Paper sx={{ p: 2, mt: 2 }}>
        <Stack spacing={2}>
          {session?.user?.image && <Avatar src={session.user.image} alt={session.user.name || "User"} sx={{ width: 80, height: 80 }} />}
              ...
              <CardContent className="p-0">
                <div className={`p-6 rounded-t-xl bg-gradient-to-br ${stat.bg}`}>
                  <div className="flex items-center justify-between">
                    <div className="h-11 w-11 rounded-lg bg-white/20 flex items-center justify-center">{stat.icon}</div>
                ...
                {[
                  ...
                  <div key={idx} className="flex items-center justify-between rounded-lg border border-gray-200 dark:border-gray-700 p-3">
                    <div>
                      ...
                      <div className={`text-xs ${item.positive ? "text-green-600 dark:text-green-400" : "text-red-600 dark:text-red-400"}`}>
                        {item.trend}
                      </div>
                    </div>
