...
const DEFAULT_FAQS = [
	...
	},
	{
		...
		answer:
			"Pencairan dana dilakukan secara transparan dan akuntabel. Penggalang dana harus mengajukan permohonan pencairan dengan melampirkan rencana Useran dana dan bukti pendukung. Tim verifikasi kami akan mereview dalam 1-3 hari kerja sebelum dana disalurkan.",
		category: "Pencairan",
	},
];
...
export default function AdminBantuanPage() {
	...
	const handleSubmit = async () => {
		...
		try {
			if (editingId) {
				...
				showSnackbar("FAQ berhasil diperbarui", "success");
			} else {
				await createFaq({ question, answer });
				showSnackbar("FAQ berhasil ditambahkan", "success");
			}
			setOpenDialog(false);
			fetchFaqs();
		} catch (error) {
			console.error(error);
			showSnackbar("Terjadi kesalahan saat menyimpan FAQ", "error");
		} finally {
		...
		setConfirmDialog({
			...
			onConfirm: async () => {
				try {
					await deleteFaq(id);
					fetchFaqs();
					showSnackbar("FAQ berhasil dihapus", "success");
				} catch (error) {
					console.error(error);
					showSnackbar("Gagal menghapus FAQ", "error");
				}
	...
	const handleSeed = () => {
		setConfirmDialog({
			...
			message: "Ini akan menambahkan data FAQ default. Lanjutkan?",
			onConfirm: async () => {
				setLoading(true);
				setConfirmDialog((prev) => ({ ...prev, open: false }));
				try {
					...
					fetchFaqs();
					showSnackbar("Default FAQ berhasil ditambahkan", "success");
				} catch (error) {
					console.error(error);
					showSnackbar("Gagal menambahkan FAQ default", "error");
				} finally {
					setLoading(false);
				}
			},
